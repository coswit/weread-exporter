...
def get_last_chapter_title(md_dir):
    """取最后一个有实际内容的章节；忽略 0 字节/无标题的残留文件"""
    if not os.path.exists(md_dir):
        return None, 0
    files = sorted((f for f in os.listdir(md_dir) if f.endswith(".md")), reverse=True)
    for fn in files:
        try:
            ...
            continue
        if title:
            return title, int(fn.replace(".md", ""))
    return None, 0
...
def save_chapter(ch_title, blocks, ch_idx, md_dir, raw_dir):
    ...
    if text_len == 0 and not img_records:
        return 0, []
    # 必须显式 utf-8：Windows 默认 GBK，遇生僻字符会在 flush 时抛 UnicodeEncodeError
    # 先写 .tmp 再替换，避免中途崩溃留下 0 字节残留
    md_path = os.path.join(md_dir, f"{ch_idx:04d}.md")
    with open(md_path + ".tmp", "w", encoding="utf-8") as f:
        f.write(body)
    os.replace(md_path + ".tmp", md_path)
    raw_path = os.path.join(raw_dir, f"{ch_idx:04d}.json")
...
async def run_session(book_id, md_dir, raw_dir, start_idx, seen_imgs,
                      ...
                      resume_idx=None, resume_title=None):
    reached_end = False
    error = False
    ...
    async def save_current(note=""):
        """把当前累积的这一章落盘并清空缓冲"""
        nonlocal ch_blocks, total_chars, total_imgs, chapters_this_session
        if not ch_blocks:
            return
        ...
        print(f"  [{ch_idx:4d}] {current_chapter[:32]:32s} {n:6d}字{img_note}{note}")
        if n or imgs:
    ...
    try:
        async with async_playwright() as p:
            ...
            # 上个会话若在这一章中途崩溃，覆盖原编号续写，避免同一章被拆成两个文件
            if resume_title and current_chapter and current_chapter == resume_title \
                    and resume_idx and not ch_blocks:
                for old in (os.path.join(md_dir, f"{resume_idx:04d}.md"),
                            os.path.join(raw_dir, f"{resume_idx:04d}.json")):
                    try:
                        os.remove(old)
                    except OSError:
                        pass
                ch_idx = resume_idx
                print(f"  ↩️  续传上次中断的章节:「{current_chapter}」(编号 {ch_idx})")
            ...
            while True:
                ...
                if new_chapter and new_chapter != current_chapter:
                    # 章节切换：保存上一章
                    await save_current(f" ({page_num}页)")
                ...
                if not got_new:
                    ...
                    if stale >= 10:
                        is_last = bool(last_cat_title and current_chapter == last_cat_title)
                        await save_current(
                            f" ({page_num}页)" + (" [全书末尾]" if is_last else ""))
                        if is_last:
                            reached_end = True
                        break
                else:
            ...
            if reached_end and ch_blocks:
                await save_current(" [末章]")

            await page.close()
            await ctx.close()
    except Exception as e:
        # 渲染进程崩溃(Target crashed)、编码异常等：保存已抓内容，交外层重开会话续传
        error = True
        first_line = str(e).splitlines()[0] if str(e) else ""
        print(f"\n  ⚠️  会话异常中断: {type(e).__name__}: {first_line[:100]}")
        try:
            await save_current("  [中断保存]")
        ...
        if ctx:
            try:
                ...
                pass
    return book_title, book_author, chapters_this_session, total_chars, ch_idx, reached_end, error
...
def download_all_images(raw_dir, img_dir):
    ...
    for jf in sorted(os.listdir(raw_dir)):
        if jf.endswith(".json"):
            for img in json.load(open(os.path.join(raw_dir, jf), encoding="utf-8")).get("images", []):
...
async def main(book_id):
    ...
    while True:
        ...
        title, author, added, chars_added, end_idx, reached_end, errored = await run_session(
            book_id, md_dir, raw_dir, start_idx, seen_imgs,
            goto_first=goto_first, catalog_path=catalog_path,
            resume_idx=last_idx if last_idx > 0 else None,
            resume_title=last_title)
        ...
        if errored:
            ...
            if consecutive_errors >= 3:
                print("\n  ❌ 连续 3 个会话异常中断，先停止。已导出内容已保留，稍后重新运行即可续传。")
                break
            print("  15 秒后重开浏览器继续..."); await asyncio.sleep(15)
            continue
        consecutive_errors = 0
        if reached_end:
            print("\n  ✅ 已到全书最后一章，导出完成。"); completed = True; break
        if added == 0:
            print("\n  无新章节，导出完成。"); completed = True; break
    ...
    if completed:
        ...
        print(f"  ⏸  导出未完成，部分章节已保存。重新运行脚本可从断点续传。")
        print(f"     📖 {book_title} — {book_author}")
